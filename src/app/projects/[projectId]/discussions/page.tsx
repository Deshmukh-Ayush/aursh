...
async function DiscussionData({ projectId }: { projectId: string }) {
  const [session, access, thread] = await Promise.all([
    getCachedSession(),
  ...
  return (
    <CommentThread 
      ...
      currentUserRole={(access.role || "agency") as any}
    ...
    <div className="flex flex-col h-[calc(100svh-3.5rem)] md:h-[calc(100svh-4rem)] max-w-4xl mx-auto w-full relative">
      <div className="shrink-0 pb-6 border-b border-border/40 mb-6">
        ...
        <p className="text-[13px] text-muted-foreground mt-1 text-pretty">
          General updates, questions, and communication for this project.
        </p>
      </div>

      <Suspense fallback={<Skeleton className="h-full w-full rounded-xl" />}>
        <DiscussionData projectId={projectId} />
      </Suspense>
    </div>
  );
}
