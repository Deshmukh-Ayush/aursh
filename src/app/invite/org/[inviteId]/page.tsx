...
import { AcceptOrgInviteButton } from "@/components/invite/accept-org-invite-button";
...
export default async function OrgInvitePage({ params }: { params: Promise<{ inviteId: string }> }) {
  ...
  const { inv, org } = invitationResult[0];
      ...
      <div className="flex min-h-svh items-center justify-center p-6 bg-muted/20">
        <Card className="max-w-md w-full">
          <CardHeader>
            ...
            <CardDescription>You have already accepted the invitation to <strong>{org.name}</strong>.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="flex min-h-svh items-center justify-center p-6 bg-muted/20">
        <Card className="max-w-md w-full">
          <CardHeader>
            ...
            <CardDescription>You&apos;ve been invited to join <strong>{org.name}</strong>.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-md bg-muted p-4 mb-4 text-sm">
              Please sign in to accept this invitation.
            </div>
            <AcceptOrgInviteButton needsLogin={true} />
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="flex min-h-svh items-center justify-center p-6 bg-muted/20">
      <Card className="max-w-md w-full border-t-4 border-t-primary shadow-lg">
        ...
        <CardContent>
          ...
          {session.user.email === inv.email ? (
            <AcceptOrgInviteButton needsLogin={false} inviteId={inviteId} />
          ) : (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground text-center">
                Please sign out and sign back in using the Google account for {inv.email}.
              </p>
              <AcceptOrgInviteButton needsLogin={true} overrideText="Sign in with correct account" />
            </div>
          )}
        </CardContent>
      </Card>
